...
1. **Create the feature.** Run `maina plan <feature-name>`. It creates a numbered directory (for example `.maina/features/012-login-rate-limit/`) with `spec.md`, `plan.md` and `tasks.md`, on a feature branch.
...
7. **Implement test-first.** Run `maina spec` to generate failing test stubs from the plan, watch them fail for the right reason, then write the minimum code to pass. Verify and commit each task (see the verify skill).

## Example

```bash
maina plan login-rate-limit
...
Add the missing task, move the Redis note to `plan.md`, check again, then `maina spec` to generate the stubs.
