 ...
 *
 * Every checklist item is a rule; an unchecked rule is a violation. A rule
 * is MUST unless its text says SHOULD. A MUST violation blocks the plan
 * unless the plan's justification table (`### Justifications` or
 * `## Complexity tracking`) has a row naming the rule with a reason. SHOULD
 * violations are recorded and never block. A plan with no gate section
 * blocks. Pure: plan text in, report out.
 */
...
export type GateViolation = Readonly<{
	rule: string;
	level: RuleLevel;
	/** 1-based line of the rule; 0 when the gate section itself is missing. */
	line: number;
