...
| `maina stats` | Show commit verification metrics, trends, and quality evolution. `--retention` shows day-7 and day-28 returns and the surface (status line, terminal notification, weekly digest) seen last before each return, from the local history in `~/.maina/retention.jsonl`; it never leaves the machine. `--retention --share` sends an anonymous summary (window statuses and per-surface counts, no dates or hosts) only if `telemetry.usage` is on in `~/.maina/policy.json`. |
