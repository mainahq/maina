...
| `maina doctor --json` | Emit `{tools, engines, aiStatus, wikiHealth, mcpHealth: { integrations: [{client, scope, fix?}] }, hostHealth: { ok, launchEnv, hosts: [{host, scope, path, command, status, checks: [{id, status, message, fix?}]}], runtime: [{id, status, message, fix?}] }, cacheStats}` for CI consumption. Exit code `1` when `hostHealth.ok` is false. |
