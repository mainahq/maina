...
3. **Scaffold** — Agent instruction files use non-destructive `<maina-managed>` regions so your existing rules stay intact:
   ...
   maina's skills are copied into `.agents/skills/`, where hosts that read Agent Skills (Codex, Cursor and others) discover them. A skill of the same name that maina did not write is left alone:
