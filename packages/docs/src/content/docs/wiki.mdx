...
The `status` tool reports whether the wiki is ready. The DeepWiki-compatible wiki tools are off by default; enable them with the allow-list (`maina --mcp --tools default,ask_question,read_wiki_structure,read_wiki_contents`, or `MAINA_MCP_TOOLS`):
