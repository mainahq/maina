...
The `mainahq/verify-action@v1` action submits your PR diff to Maina Cloud for verification. No tool installation required on the runner.
...
Parse the JSON output for custom integrations. The `--json` flag returns a `{ data, error, meta }` envelope with all findings.
