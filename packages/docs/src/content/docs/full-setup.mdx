...
<Tabs>
  <TabItem label="Wizard (recommended)">
    ```bash
    bunx @mainahq/cli@latest setup
    ```
    Detects your stack, generates a tailored constitution, scaffolds agent files, seeds the wiki, and runs verify — no API key, no login. See [Getting Started](/getting-started).
  </TabItem>
  <TabItem label="bun">
    ```bash
  ...
  <TabItem label="One-liner (shell)">
    ```bash
    curl -fsSL https://api.mainahq.com/install | bash
    ```
    Detects your OS and installs maina globally. Pair with `maina setup`.
  </TabItem>
</Tabs>
