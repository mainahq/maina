---
title: Verify Engine
description: Verification pipeline with syntax guard, parallel tools, diff-only filtering, and two-stage review.
---

The Verify Engine proves code is correct. Static analysis tools find issues. AI generates fixes. Humans review. Feedback improves everything.
...
The solution: a hybrid verification pipeline that runs static analysis tools first, filters to changed lines only, and uses AI only for fix generation and review.
...
Parallel Static Analysis
    ...
    |   consistency (cross-function pattern check)
    |
    v
...
### Findings and review triage

Every finding gets a `realProbability`: how likely it is to be a real problem, judged from how often you dismissed that rule before. A finding Maina is confident is noise (at or above the policy's `finding.real` confidence threshold, 0.8 by default) is dropped; one that is more likely noise than real is shown one severity lower. Probabilities, not raw dismiss ratios, drive both.

The diff is triaged too. A large diff, a wide one, or one that touches security-sensitive paths (auth, sessions, secrets, tokens, payments, migrations) gets the standard-tier AI review without `--deep`; everything else gets the fast mechanical one. If the triage cannot decide, the standard-tier review runs. The receipt records the triage decision (`triage: { decisionId, needsReview, confidence }`), and the AI review sees the bodies of the functions your diff calls, read from the code graph.
...
| **consistency** | Pattern-based cross-function consistency check | Built-in |
...
See the [CI Integration](/ci/) docs for full configuration options.
