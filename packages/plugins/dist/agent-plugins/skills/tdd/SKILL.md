...
1. **Generate test stubs** with `../../launcher/launch.sh cli spec`. This reads the current feature's plan.md and creates test files for each task, organized by test category.
...
6. **Verify the cycle** with `../../launcher/launch.sh cli verify` (or call the `verify` MCP tool) to confirm all deterministic checks pass on your changes.
...
- The `../../launcher/launch.sh cli analyze` command checks that test tasks (T001: Write tests for X) appear before their implementation tasks (T002: Implement X) in the plan.
