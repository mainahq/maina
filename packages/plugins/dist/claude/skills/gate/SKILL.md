...
4. **Ask before you act, when unsure.** Call the `decide` MCP tool with type `action.risk` and the action as state, or run `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli decide --type action.risk --json`, to get the policy's verdict and confidence without running anything.
