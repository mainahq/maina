...
4. **Ask before you act, when unsure.** Call the `decide` MCP tool with type `action.risk`, one choice question with the options `allow`, `ask` and `deny`, and the action's class as `state.trusted.actionClass` (a key of `action_classes` in the policy, such as `fs.delete.recursive` or `git.push.force`), or run `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli decide --type action.risk --trusted actionClass=<class> --json`. It answers with the policy's verdict for that class and its confidence, without running anything; a class the policy does not know answers ask. The gate still judges the concrete command when it runs, so its rules can answer differently.
5. **Check the gate is live.** The `status` MCP tool reports whether the policy is loaded and valid; `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli doctor` checks the policy file and the MCP setup, and prints a fix for each failed check.
