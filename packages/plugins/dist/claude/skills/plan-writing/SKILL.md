...
1. **Start with brainstorm** using `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli brainstorm` to explore the idea, surface edge cases, and generate a structured GitHub issue. This is the recommended starting point before planning.
2. **Scaffold the feature** with `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli plan <name>`. This creates a numbered feature directory under `.maina/features/` containing `spec.md` and `plan.md` templates.
...
5. **Check consistency** with `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli analyze` (or call the `spec_check` MCP tool on the feature directory). This verifies:
...
6. **Generate test stubs** with `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli spec`. This reads the plan and creates failing test files for each task, ready for the TDD red-green-refactor cycle.
...
"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli plan user-auth
...
"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli analyze
...
# Generate test stubs
"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli spec
...
- The `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli analyze` check is also run automatically during `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli verify`.
