...
5. **Run the affected tests.** After the change, run `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli verify --tests`: verify plus the tests the graph links to your change (see the verify skill).
...
Nine dependent files is a wide reach: add an optional parameter with a default rather than changing the signature, then run `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli verify --tests`.

## Notes

- The graph is built from the syntax tree of the source, so it sees real calls and imports, not text matches.
- For questions about why the code is shaped as it is, the maina wiki answers with cited articles: `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli wiki query "<question>"`, or the `ask_question` MCP tool when the server enables it.
