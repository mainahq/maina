...
1. **Run context generation** with `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli context` (or call the `context` MCP tool for explicit files or a query). This activates all four retrieval layers and returns a structured context document.
...
"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli context

# Focused context for a specific file change
"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli context --mode focused

# Broad exploration of a subsystem
"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli context --mode explore
...
- Each maina command declares its own context needs via a selector, so `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli review` and `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli plan` automatically request different context shapes.
- The semantic layer uses tree-sitter for language-aware parsing, supporting TypeScript, Python, Go, Rust, C#, Java, PHP, and more.
- When logged into Maina Cloud (`"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli login`), the episodic layer merges team entries automatically -- reviews your teammates accepted feed into your context. Entries are deduplicated by content hash and decay naturally over time.
