...
1. **Generate test stubs** with `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli spec`. This reads the current feature's plan.md and creates test files for each task, organized by test category.
...
6. **Verify the cycle** with `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli verify` (or call the `verify` MCP tool) to confirm all deterministic checks pass on your changes.
...
- The `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli analyze` command checks that test tasks (T001: Write tests for X) appear before their implementation tasks (T002: Implement X) in the plan.
