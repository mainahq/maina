...
1. **Create the feature.** Run `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli plan <feature-name>`. It creates a numbered directory (for example `.maina/features/012-login-rate-limit/`) with `spec.md`, `plan.md` and `tasks.md`, on a feature branch.
...
7. **Implement test-first.** Run `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli spec` to generate failing test stubs from the plan, watch them fail for the right reason, then write the minimum code to pass. Verify and commit each task (see the verify skill).

## Example

```bash
"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli plan login-rate-limit
...
Add the missing task, move the Redis note to `plan.md`, check again, then `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli spec` to generate the stubs.
