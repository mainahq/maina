...
5. **Check what the change can break.** Run `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli verify --tests` to also run the tests the code graph links to your change (see the graph skill).
6. **Commit through maina.** Stage the files by name and run `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli commit -m "<conventional message>"`. It verifies the staged change again and adds a `Verified-by` trailer.
7. **Produce a receipt when asked.** `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli receipt` runs the pipeline and writes a receipt (JSON and HTML) under `.maina/receipts/`. Check receipts with the `receipt` MCP tool or `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli verify-receipt <path>`.
...
"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli commit -m "fix(auth): reject logins without a password"
```

## Notes

- Only changed lines are reported, so existing debt in untouched code never blocks you.
- `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli verify --staged` checks only the staged changes; `--base <ref>` picks the branch to diff against; `--deep` forces the AI semantic review on top of the deterministic tools.
