...
1. **Run the review** with `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli review` (or call the `review_triage` MCP tool). This performs a two-stage AI-powered review on the current diff.
...
```bash
"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli review
...
- Combine with `"${CLAUDE_PLUGIN_ROOT}/launcher/launch.sh" cli verify` (or the `verify` MCP tool) for the complete pre-merge pipeline: deterministic tools first, then AI review.
