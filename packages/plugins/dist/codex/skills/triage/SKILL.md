...
1. **Review the diff.** Call the `review_triage` MCP tool with the `files` you changed and a `base` ref (or a `diff`), and pass the feature's plan as `planContent` when there is one. From a terminal, run `../../launcher/launch.sh cli review --base <ref> --plan <path-to-plan.md>`.
