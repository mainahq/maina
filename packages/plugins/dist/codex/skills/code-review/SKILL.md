...
1. **Run the review** with `../../launcher/launch.sh cli review` (or call the `review_triage` MCP tool). This performs a two-stage AI-powered review on the current diff.
...
- Combine with `../../launcher/launch.sh cli verify` (or the `verify` MCP tool) for the complete pre-merge pipeline: deterministic tools first, then AI review.
