...
- Every receipt has a `retries: number` field (v1 schema, see adr/0030).
...
- Retry count can be spoofed if an attacker controls both the agent and the receipt generator. Out of scope for v1 (integrity-only hashing per adr/0030); mitigated in v2 by keypair-signed receipts tied to the CI runner identity.
...
- Sibling schema ADR: adr/0030-receipt-v1-field-schema.md (defines the `retries` field)
